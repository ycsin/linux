...
static void dbg_dumpregs(struct ftsdc_host *host, char *prefix)
{
	u32 con, cmdarg, r0, r1, r2, r3, rcmd, dcon, dtimer,
	    dlen, sta, clr, imask, pcon, ccon, bwidth, scon1, scon2, ssta, fea;
	...
	dbg(host, dbg_debug, "%s DCON:[%08x] DTIME:[%08x] DLEN:[%08x] DWIDTH:[%08x]\n", prefix, dcon, dtimer, dlen, bwidth);

	dbg(host, dbg_debug, "%s R0:[%08x] R1:[%08x] R2:[%08x] R3:[%08x]\n", prefix, r0, r1, r2, r3);

	dbg(host, dbg_debug, "%s SCON1:[%08x] SCON2:[%08x] SSTA:[%08x]   FEA:[%08x]\n", prefix, scon1, scon2, ssta, fea);
}
...
#ifdef CONFIG_MMC_DEBUG
	if (imask & SDC_STATUS_REG_SDIO_INTR)
		pr_err("\n*** E ***\n");
#endif
...
static inline void get_data_buffer(struct ftsdc_host *host)
{
	...
	host->buf_ptr =
	    host->dodma ? (u32 *) (unsigned long)sg->dma_address : sg_virt(sg);
	...
	while (host->buf_sgptr < host->mrq->data->sg_len) {
		...
		while (host->buf_bytes) {
			...
			if (host->buf_page != 0) {
				...
				host->page_cnt =
				    min((u32) (PAGE_SIZE - host->buf_offset),
					host->buf_bytes);
				host->buf_offset = 0;
			}
...
static void do_dma_access(struct ftsdc_host *host, struct mmc_data *data)
{
	...
	while (host->buf_sgptr < host->mrq->data->sg_len) {
		...
		if (res != 0 || (drb == 0)) {
			dbg(host, dbg_err,
			    "%s() Failed to allocate dma request block!\n",
			    __func__);
		...
		if (res != 0) {
			dbg(host, dbg_err,
			    "%s() Failed to submit dma request block!\n",
			    __func__);
			host->mrq->data->error = -ENODEV;
			goto err;
		}
...
{
	struct ftsdc_host *host = container_of(work, struct ftsdc_host, work);
...
static void ftsdc_send_command(struct ftsdc_host *host, struct mmc_command *cmd)
{
	u32 ccon = 0;
	u32 newmask = 0;
	u32 scon;
...
static int ftsdc_setup_data(struct ftsdc_host *host, struct mmc_data *data)
{
	...
	if (!data)
		return 0;
	if (host->mmc->card
	    && host->mmc->card->type == (unsigned int)MMC_TYPE_SD) {
		if (((data->blksz - 1) & data->blksz) != 0) {
			pr_warn
			    ("%s: can't do non-power-of 2 sized block transfers (blksz %d)\n",
			     __func__, data->blksz);
			return -EINVAL;
		}
	}
	...
	if (ftsdc_dmaexist(host) && ((data->blksz * data->blocks) & 0xf) == 0) {
		newmask &= ~SDC_INT_MASK_REG_FIFO_OVERRUN;
	...
	/* handle sdio */
	dcon =
	    SDC_SDIO_CTRL1_REG_READ_WAIT_ENABLE & REG_READ(SDC_SDIO_CTRL1_REG);
...
static void ftsdc_set_clk(struct ftsdc_host *host, struct mmc_ios *ios)
{
	...
	if (ios->clock == 0) {
		...
		con =
		    (con & ~SDC_CLOCK_CTRL_REG_CLK_DIV) | (clk_div &
							   SDC_CLOCK_CTRL_REG_CLK_DIV);
		con &= ~SDC_CLOCK_CTRL_REG_CLK_DIS;
	}
...
static void ftsdc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	con &= ~(SDC_BUS_WIDTH_REG_SINGLE_BUS |
		 SDC_BUS_WIDTH_REG_WIDE_4_BUS | SDC_BUS_WIDTH_REG_WIDE_8_BUS);
...
static void ftsdc_enable_sdio_irq(struct mmc_host *mmc, int enable)
{
	...
	con =
	    enable ? (con | SDC_STATUS_REG_SDIO_INTR) : (con &
							 ~SDC_STATUS_REG_SDIO_INTR);
...
*mmc_parse_pdata(struct platform_device *pdev)
{
	...
	if (!pdata) {
		dev_err(&pdev->dev,
			"Failed to allocate memory for struct ftsdc_mmc_config\n");
		goto nodata;
	}
