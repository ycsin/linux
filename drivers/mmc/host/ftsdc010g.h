...
struct ftsdc_host {
	...
	/* keep last successful cmd to judge application specific command */
	...
	bool			dodma;
	bool			dma_finish;
	/* keep next scallterlist buffer index */
	u32			buf_sgptr;
	/* keep current total scallterlist buffer length */
	u32			buf_bytes;
	/* keep real data size rw from sd */
	u32			buf_count;
	/* keep current scallterlist buffer address */
	u32			*buf_ptr;
	u32			page_cnt;
